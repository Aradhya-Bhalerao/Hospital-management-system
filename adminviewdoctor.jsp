...
<%
    ...
    if (request.getParameter("action") != null && request.getParameter("action").equals("delete")) {
        String doctorIdToDelete = request.getParameter("id");
        ...
        try {
            ...
            deleteStatement.setString(1, doctorIdToDelete);
            int rowsAffected = deleteStatement.executeUpdate();
            if (rowsAffected > 0) {
                out.println("<script>alert('Doctor deleted successfully.');</script>");
            } else {
                out.println("<script>alert('Failed to delete doctor.');</script>");
            }
...
<html>
    ...
    <body>
            ...
            <table id="doctorTable">
                ...
                <%
                    try {
                        ...
                        String sql = "SELECT id, name,status FROM doctor";
                ...
                <tr>
                    ...
                    <td>
                         <select onchange="changeStatus(this.value, '<%= id%>')">
        ...
        <option value="inactive" <%= rs.getString("status").equals("inactive") ? "selected" : "" %>>Inactive</option>
                    ...
                    <td class="update"><a href="updatedoctorpassword.jsp" style="text-decoration:none; background-color: lightgreen; border: 1px solid black; border-radius:10px; padding: 5px; color:black;">Update</a></td>
                    <td class="delete"><a href="?action=delete&id=<%= id%>" onclick="return confirm('Are you sure you want to delete this doctor?')" style="text-decoration:none; background-color: red; border: 1px solid black; border-radius:10px; padding: 5px; color:black;">Delete</a></td>
        ...
        <script>
            ...
            function changeStatus(status, doctorId) {
    fetch('changeStatus.jsp?id=' + doctorId + '&status=' + status)
        .then(response => response.text())
        .then(data => {
            alert(data); // Display success or error message
            // Optionally, you can update the UI to reflect the changed status
        })
